...
void main() async {
  ...
  final uid = await authService.checkSignIn();

  String initialRoute = LoginScreen.routeName;
  if (uid != null) {
    // Fetch user profile
    final userProfile = await authService.fetchUserProfile(uid);
    if (userProfile != null) {
  ...
  runApp(
    ChangeNotifierProvider<ThemeProvider>.value(
      value: themeProvider,
      child: BookHub(initialRoute: initialRoute),
    ),
  );
}
...
class BookHub extends StatelessWidget {
  ...
  const BookHub({Key? key, required this.initialRoute}) : super(key: key);
